...
async function main() {
  ...
  check(index.includes('aria-label="Zoom out"') && index.includes('aria-label="Zoom in"') && index.includes('aria-label="Photo mode"') && index.includes('aria-label="Turn on ambient garden music"') && index.includes('aria-label="Open visitor message board"'), 'mobile icon controls should expose accessible names');
  check(index.includes('name="viewport"') && !index.includes('user-scalable=no') && !index.includes('maximum-scale=1'), 'mobile viewport should allow browser zoom');
  ...
  check(index.includes('id="history-toggle" type="button"') && index.includes('aria-controls="history-panel" aria-expanded="false"') && index.includes('id="stats-toggle" type="button"') && index.includes('id="memory-toggle" type="button"') && index.includes('syncInfoPanelControls') && index.includes('aria-label="Close garden history"') && index.includes('aria-label="Close garden stats"') && index.includes('aria-label="Close garden memory"'), 'history, stats, and memory controls should expose keyboard buttons, synchronized expanded state, and named close actions');
  check(index.includes('aria-label="Close story guide"') && index.includes('aria-label="Close message board"') && index.includes('aria-label="Close object details"') && index.includes('aria-label="Close story log"'), 'icon-only overlay close buttons should expose accessible names');
  check(index.includes('id="msg-board-btn" type="button"') && index.includes('aria-controls="msg-board" aria-expanded="false"') && index.includes("button.setAttribute('aria-expanded', String(msgBoardVisible))") && index.includes("msgBoardVisible ? 'Close visitor message board' : 'Open visitor message board'"), 'message board control should expose its target, expanded state, and current action');
  ...
  check(index.includes('const workerFrame = prefersReducedMotion ? 0 : frameCount'), 'civic worker motion should pause in calm mode');
  check(index.includes('function primitiveWorkCue(site)') && index.includes("if (site.type === 'tool-yard') return 'TOOL';") && index.includes('drawPrimitiveWorkCue(site, x, y);'), 'first-camp work sites should label their visible civic job');
